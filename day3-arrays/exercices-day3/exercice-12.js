...
function removeDoubles(tableau){
    const clearTable = tableau;
    for(let i = 0; i<clearTable.length; i++){
        for(let j = i+1; j<clearTable.length; j++){
            if(clearTable[i] == clearTable[j]){
                clearTable.splice(j, 1);
                j--;
            }
        }
    }
...
console.log(removeDoubles(tableau));