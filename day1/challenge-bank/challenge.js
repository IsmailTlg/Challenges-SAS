...
let dureeEnMois = 24; 
let revenuMensuel = 10001;
let mensualiteEstime = montantEmprunte/dureeEnMois;
if(mensualiteEstime <= (0.3 * revenuMensuel)){
    console.log(`montant: ${montantEmprunte}, ${dureeEnMois} mois (mensualite: ${mensualiteEstime})`);
    console.log(`30% de ${revenuMensuel} = ${0.3 * revenuMensuel}`)
    console.log(`Donc le Pret est accorde. Mensualite: ${mensualiteEstime} MAD`)   
}
else{
    console.log(`montant: ${montantEmprunte}, ${dureeEnMois} mois (mensualite: ${mensualiteEstime})`);
    console.log(`30% de ${revenuMensuel} = ${0.3 * revenuMensuel}`)
    if(revenuMensuel > 10000){
        while(mensualiteEstime >= 0.3 * revenuMensuel){
            dureeEnMois++;
            mensualiteEstime = montantEmprunte/dureeEnMois;
            
        }
        console.log(`comme le revenu > 10000, vous pouvez augmentez le duree a ${dureeEnMois} mois`);
    }
    else{
        console.log("Comme le revennu est < 10000 le pret est refuse")
    }
