...
function reparerPrix(liste){
    //let propretie = Object.keys(liste)
    //let price = propretie[1];
    for(let i = 0; i<liste.length; i++){
        liste[i].prix = liste[i].prix.slice(0, -1);
        liste[i].prix = Number(liste[i].prix);
    }
    return liste;

}
let liste = [{ article: "PC", prix: "1500$" }, { article: "Souris", prix: "25$" }];
console.log(reparerPrix(liste))